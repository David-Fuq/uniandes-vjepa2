...
from app.vjepa_droid.utils import init_opt
...
def train(args, verbose=False):
    ...
    target_encoder = copy.deepcopy(initial_encoder)
    ...
    logger.info("Verifying target_encoder is frozen:")
    total_params = 0
    frozen_params = 0
    trainable_params = 0
    for name, p in target_encoder.named_parameters():
        total_params += 1
        if p.requires_grad:
            trainable_params += 1
            logger.warning(f"Parameter NOT frozen: {name}")
        else:
            frozen_params += 1

    logger.info(f"Target encoder: {total_params} total params, {frozen_params} frozen, {trainable_params} trainable")
    if trainable_params > 0:
        logger.error("ERROR: Target encoder is NOT fully frozen!")
    else:
        logger.info("✓ Target encoder successfully frozen")

    # Also check encoder and predictor status
    ...
    logger.info(f"  {encoder_trainable:,} / {encoder_total:,} ({100*encoder_trainable/encoder_total:.1f}%)")

    logger.info("\nPredictor trainable parameters:")
    predictor_trainable = sum(p.numel() for p in predictor.parameters() if p.requires_grad)
    predictor_total = sum(p.numel() for p in predictor.parameters())
    logger.info(f"  {predictor_trainable:,} / {predictor_total:,} ({100*predictor_trainable/predictor_total:.1f}%)")
    ...
    for epoch in range(num_epochs):
        ...
        for itr in range(ipe): #Iterations per epoch
            ...
            def load_clips():
                ...
                video_path = sample[3] if len(sample) > 3 else None  
                return (clips, actions, states, video_path)

            clips, actions, states, video_path = load_clips()
            data_elapsed_time_ms = (time.time() - itr_start_time) * 1000.0

            if video_path is not None:
                logger.info(f"Processing video [{itr}/{ipe}]: {video_path[0] if isinstance(video_path, (list, tuple)) else video_path}")
            ...
            def train_step():
                ...
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    ...
                    loss = (jloss + sloss)/accumulation_steps #To scale loss by accumulation steps
                ...
                return (
                    float(loss) * accumulation_steps, #Unscaled loss for logging
                    float(jloss),
                    float(sloss),
                    _new_lr,
                    _new_wd,
