...
checkpoint_dir = os.path.join(main_dir, "final_dataset_40_epoch_original")
...
# Set device - use GPU 1 since GPU 0 is busy
...
np_clips_full = trajectory["observations"][:, :4]
np_states_full = trajectory["states"][:, :4]
if play_in_reverse:
...
plt.savefig(os.path.join(script_dir, "trajectory_frames_local_test_2.png"), bbox_inches='tight', dpi=100)
print(f"\nSaved visualization to {os.path.join(script_dir, 'trajectory_frames_local_test_2.png')}")
plt.close()
...
plt.savefig(os.path.join(script_dir, "energy_landscape_local_test_2.png"), bbox_inches='tight', dpi=150)
print(f"\nGround truth action frame 1→2: (x={gt_x:.4f}, y={gt_y:.4f}, z={gt_z:.4f})")
print(f"Saved energy landscape to {os.path.join(script_dir, 'energy_landscape_local_test_2.png')}")
plt.close()
...
# Compute ground truth actions for comparison
...
for i in range(rollout_steps):
    planned = planned_actions[i]
    
    # Compute ground truth action from states and convert to numpy
    gt_action = poses_to_diff(
        np_states_full[0, i],
        np_states_full[0, i+1]
    ).numpy()  # Add .numpy() here
    
    # Compute error (L2 norm on xyz only)
    error = np.linalg.norm(planned[:3] - gt_action[:3])
    total_error += error
    
    print(f"{i+1:<6} ({planned[0]:6.4f},{planned[1]:6.4f},{planned[2]:6.4f})      "
