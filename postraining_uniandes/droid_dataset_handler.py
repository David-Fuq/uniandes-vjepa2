...
Module for handling the Droid dataset.

This module attends batches of data from the Droid dataset, providing
functionality to load, preprocess, and iterate over the dataset efficiently.

It will give the videos, actions, states and other relevant information
needed for the postraining tasks.

This is a simplified version of the app/vjepa_droid/droid.py file from the vjepa_droid repository.

Important changes/simplifications:
- No use of extrenciscs at all
...
#Metadata loader. This is the metadata_RAD+c6cf6b42+2023-11-18-15h-53m-04s.json for example. 
...
class DROIDVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        self,
        data_path,
        camera_views=["left_mp4_path"], #Only left camera view. It is stated in the paper that using only one camera view is better.
        frameskip=2, #Not sure why they skip a frame... but will keep it this way for now, trying to have same results as META.
        frames_per_clip=16,
        fps=5,
        transform=None,
        camera_frame=False,
        ...
        logger.info(f"Number of samples in DROID dataset: {len(samples)}")
        self.samples = samples
    # Gets one single sample from the dataset
    def __getitem__(self, index):
        ...
        while not loaded_video:
            try:
                buffer, actions, states, indices = self.loadvideo_decord(path)
                logger.info(f"Currently checking the video: {path[-15:]}")
                loaded_video = True
            except Exception as e:
        ...
        return buffer, actions, states, indices

    # Converts absolute poses to relative differences.
    ...
    # Transforms the poses to the camera frame using the extrinsics.
    # Will not be used considering extrinsics are not used in the paper experiments.
    ...
    # Actual process of loading a video sample.
    # Same as original code, except no use of extrinsics. All extrinsics related lines are commented.
